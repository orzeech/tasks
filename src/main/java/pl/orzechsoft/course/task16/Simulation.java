...
public class Simulation {
  ...
  private static List<RaceCar> simulate(List<RaceCar> cars, List<Integer> track) {
    ...
    for (RaceCar car : cars) {
      ...
      double acceleration = car.getTransmission().getAcceleration() * car.getEngine()
          .getAccelerationPenalty();
      double deceleration =
          -1 * car.getBrakes().getDeceleration() * car.getEngine().getAccelerationPenalty();
      ...
      for (int straightPart : track) {
        ...
        boolean canReachTurnSpeed = currentSpeed >= maxTurnSpeed
            || (maxTurnSpeed * maxTurnSpeed - currentSpeed * currentSpeed) / (2 * acceleration)
            < straightPart;
        if (!canReachTurnSpeed) {
          ...
          if (v > maxSpeed) {
            ...
            double partAtConstantSpeed =
                0.5 * acceleration * t1 * t1 + 0.5 * Math.abs(deceleration) * t2 * t2;
